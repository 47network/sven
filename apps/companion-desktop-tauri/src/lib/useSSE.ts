...
export function handleSSEEvent(ev: SSEEvent, o: SSEOptions) {
    switch (ev.type) {
        case 'message': {
            ...
            if (item.role !== 'user') {
                showNotification('New message from Sven', {
        ...
        case 'approval': {
            ...
            showNotification('Sven approval pending', {
        ...
        case 'call_signal':
            ...
            if ((ev.data.type as string) === 'offer') {
...
export function createEventSource(
    ...
    const es = new EventSource(url, {} as EventSourceInit);
...
export function useSSE(opts: SSEOptions) {
    ...
    const connect = useCallback(() => {
        ...
        esRef.current = createEventSource(
            url,
            (ev) => handleSSEEvent(ev, optsRef.current),
            () => {
                setConnected(true);
