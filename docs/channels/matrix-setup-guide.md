...
This guide covers baseline setup for services/adapter-matrix.

## Prerequisites

- Sven core stack running (gateway-api, agent-runtime, nats, postgres).
- Channel credentials/tokens provisioned for matrix.
...
3. Ensure egress/network policy allows the matrix platform endpoint.

## Bring-up

1. Start adapter-matrix with the rest of services.
