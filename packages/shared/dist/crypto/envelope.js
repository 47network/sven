 ...
 */
export function deriveDekFromMasterKey(masterKey, salt, algorithm = 'aes-256-gcm', iterations = 100000) {
 ...
 */
export function wrapDataKey(dek, masterKey, salt, algorithm = 'aes-256-gcm', iterations = 100000) {
    if (algorithm !== 'aes-256-gcm') {
        throw new Error(`Unsupported algorithm: ${algorithm}`);
    }
 ...
 */
export function unwrapDataKey(wrappedKey, masterKey, algorithm = 'aes-256-gcm') {
 ...
 */
export function encryptData(plaintext, dek, algorithm = 'aes-256-gcm', aad) {
    if (algorithm !== 'aes-256-gcm') {
        throw new Error(`Unsupported algorithm: ${algorithm}`);
    }
 ...
 */
export function decryptData(encrypted, dek) {
 ...
 */
export function generateDek(keyLength = 32) {
 ...
 */
export async function encryptUserData(plaintext, dek, userId, contentType = 'user_private_memory', logger) {
    try {
 ...
 */
export async function decryptUserData(encrypted, dek, userId, contentType = 'user_private_memory', logger) {
    try {
        const aad = `${userId}:${contentType}`;
