...
function isSubjectsOverlapError(error: unknown): boolean {
  if (!(error instanceof Error)) {
    return false;
  }
...
function isWorkqueuePolicyChangeError(error: unknown): boolean {
  if (!(error instanceof Error)) {
    return false;
  }
...
export async function ensureStreams(connection: NatsConnection): Promise<void> {
