...
let overrideRedactionConfig: RedactionConfig | null = null;
...
function parseCustomPatterns(raw: string | undefined): RegExp[] {
...
function currentRedactionConfig(): RedactionConfig {
  if (overrideRedactionConfig) return overrideRedactionConfig;
  return {
...
export function setLoggerRedactionConfig(config: Partial<{
  redactSensitive: boolean;
  patterns: string[];
}> | null): void {
  if (!config) {
...
function redactUnknownValue(value: unknown, config: RedactionConfig): unknown {
  if (value === null || value === undefined) return value;
  if (typeof value === 'string') return redactStringValue(value, config);
  if (typeof value === 'number' || typeof value === 'boolean') return value;
  if (Array.isArray(value)) return value.map((entry) => redactUnknownValue(entry, config));
...
export function redactLogEntry(entry: LogEntry): LogEntry {
  ...
  if (!config.redactSensitive && config.customPatterns.length === 0) return entry;
  return redactUnknownValue(entry, config) as LogEntry;
}
