...
export function encryptLiteLlmVirtualKey(value: string): string {
...
export function decryptLiteLlmVirtualKey(value: string): string {
  const input = String(value || '').trim();
  if (!input) return '';
  if (!input.startsWith(PREFIX)) return input;
  ...
  if (!ivPart || !tagPart || !encryptedPart) return input;
