...
Write-Host "Running privacy compliance check against $ApiUrl ..."
node scripts/privacy-compliance-check.cjs
$exitCode = [int]$LASTEXITCODE
if ($exitCode -ne 0) {
  throw "privacy compliance check failed with exit code $exitCode"
}

Write-Host "Latest status from current run:"
Get-Content docs/release/status/privacy-compliance-latest.md
