...
if (-not (Test-Path $logPath)) {
  Write-Error "Missing multi-device evidence log: $logPath"
}
...
$outJson = Join-Path $RepoRoot 'docs/release/status/multi-device-validation-latest.json'
$outMd = Join-Path $RepoRoot 'docs/release/status/multi-device-validation-latest.md'
$payload = [ordered]@{
...
@("# Multi-device Validation","","Generated: $($payload.generated_at)","Status: $status","","- total: $($payload.total)","- failed: $($payload.failed)") | Set-Content -Encoding UTF8 $outMd
