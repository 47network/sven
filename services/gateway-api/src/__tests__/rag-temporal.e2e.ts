import http from 'http';
import { describe, expect, it } from '@jest/globals';

const API_BASE = process.env.API_URL || 'http://127.0.0.1:3001';
const TEST_SESSION_COOKIE = process.env.TEST_SESSION_COOKIE || '';

type ApiResult = {
  statusCode: number;
  data: unknown;
  raw: string;
  headers: http.IncomingHttpHeaders;
};

async function apiCall(
  method: string,
  endpoint: string,
  body?: unknown,
  opts?: { bearer?: string; cookie?: string },
): Promise<ApiResult> {
  return new Promise((resolve, reject) => {
    const parsed = new URL(`${API_BASE}${endpoint}`);
    const payload = body ? JSON.stringify(body) : '';
    const headers: Record<string, string> = {};
    if (payload) {
      headers['content-type'] = 'application/json';
      headers['content-length'] = String(Buffer.byteLength(payload));
    }
    if (opts?.bearer) headers.authorization = `Bearer ${opts.bearer}`;
    if (opts?.cookie) headers.cookie = opts.cookie;

    const req = http.request(
      {
        method,
        hostname: parsed.hostname,
        port: parsed.port,
        path: parsed.pathname + parsed.search,
        headers,
      },
      (res) => {
        let raw = '';
        res.on('data', (chunk) => {
          raw += String(chunk);
        });
        res.on('end', () => {
          const contentType = String(res.headers['content-type'] || '');
          let parsedBody: unknown = null;
          if (contentType.includes('application/json')) {
            try {
              parsedBody = raw ? JSON.parse(raw) : {};
            } catch {
              parsedBody = null;
            }
          }
          resolve({
            statusCode: res.statusCode || 0,
            data: parsedBody,
            raw,
            headers: res.headers,
          });
        });
      },
    );
    req.on('error', reject);
    if (payload) req.write(payload);
    req.end();
  });
}

async function getBearerFromSessionCookie(cookie: string): Promise<string> {
  const started = await apiCall('POST', '/v1/auth/device/start', {
    client_name: `D4 rag temporal ${Date.now()}`,
    client_type: 'ci',
    scope: 'd4 rag temporal',
  });
  expect(started.statusCode).toBe(200);
  const deviceCode = String((started.data as { data?: { device_code?: unknown } })?.data?.device_code || '');
  const userCode = String((started.data as { data?: { user_code?: unknown } })?.data?.user_code || '');
  expect(deviceCode).toBeTruthy();
  expect(userCode).toBeTruthy();

  const confirmed = await apiCall('POST', '/v1/auth/device/confirm', { user_code: userCode }, { cookie });
  expect(confirmed.statusCode).toBe(200);

  const tokenResp = await apiCall('POST', '/v1/auth/device/token', { device_code: deviceCode });
  expect(tokenResp.statusCode).toBe(200);
  const token = String((tokenResp.data as { data?: { access_token?: unknown } })?.data?.access_token || '');
  expect(token).toBeTruthy();
  return token;
}

describe('D4 temporal RAG first slice', () => {
  it('flags stale chunks and exposes temporal metadata in search results', async () => {
    if (!TEST_SESSION_COOKIE) {
      expect(true).toBe(true);
      return;
    }

    const bearer = await getBearerFromSessionCookie(TEST_SESSION_COOKIE);
    const unique = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    const needle = `temporal-rag-${unique}`;
    const staleDate = '2020-01-01T00:00:00.000Z';
    const freshDate = new Date().toISOString();

    const oldIngest = await apiCall(
      'POST',
      '/v1/admin/rag/ingest/structured',
      {
        source: `api://old-${unique}`,
        source_type: 'api',
        dataset_name: 'temporal-old',
        source_updated_at: staleDate,
        rows: [{ id: 1, note: needle, state: 'old' }],
        visibility: 'global',
      },
      { bearer },
    );
    expect([200, 503]).toContain(oldIngest.statusCode);
    if (oldIngest.statusCode === 503) return;

    const freshIngest = await apiCall(
      'POST',
      '/v1/admin/rag/ingest/structured',
      {
        source: `api://fresh-${unique}`,
        source_type: 'api',
        dataset_name: 'temporal-fresh',
        source_updated_at: freshDate,
        rows: [{ id: 2, note: needle, state: 'fresh' }],
        visibility: 'global',
      },
      { bearer },
    );
    expect(freshIngest.statusCode).toBe(200);

    const search = await apiCall(
      'POST',
      '/v1/admin/rag/search',
      {
        query: needle,
        source_types: ['api'],
        modalities: ['structured'],
        prefer_recent: true,
        stale_after_days: 30,
        top_n: 10,
        top_k: 10,
      },
      { bearer },
    );
    expect(search.statusCode).toBe(200);

    const payload = (search.data as { data?: { results?: unknown[]; meta?: Record<string, unknown> } })?.data || {};
    const rows = Array.isArray(payload.results) ? payload.results as Array<Record<string, unknown>> : [];
    const meta = payload.meta || {};

    expect(Array.isArray(rows)).toBe(true);
    expect(Boolean(meta.temporal)).toBe(true);
    expect(Boolean((meta.temporal as Record<string, unknown>)?.prefer_recent)).toBe(true);

    if (rows.length > 0) {
      const firstTemporal = rows[0]?.temporal as Record<string, unknown> | undefined;
      expect(Boolean(firstTemporal)).toBe(true);
      expect(typeof firstTemporal?.is_stale).toBe('boolean');
    }
  });
});

