...
async function seed() {
  ...
  try {
    ...
    const svenCoreSoulContent = [
      ...
      '',
      '  **Network**: All VMs connected via WireGuard mesh (wg0 interface). TLS 1.2+ enforced on all cross-VM database connections. VM1 (10.47.47.5) is the edge L4/L7 reverse proxy with SNI routing — terminates TLS for the47network.com and proxies to VM14. Nginx on VM4 terminates public TLS for sven.systems domains. Docker networks: multi-vm_platform (bridge), core, tools.',
      '',
